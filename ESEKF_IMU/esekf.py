...
import math
from ESEKF_IMU import transformations as tr
...
class ESEKF(object):
    def __init__(self, init_nominal_state: np.array, imu_parameters: ImuParameters, last_predict_time=0.0):
        """
        ...
        self.last_predict_time = last_predict_time
    ...
    def update(self, gt_measurement: np.array, measurement_covar: np.array):
        ...
        H = np.zeros((6, 18))
        H[0:3, 0:3] = np.eye(3)
        H[3:6, 3:6] = np.eye(3)  # 测量矩阵
        PHt = self.error_covar @ H.T  # 18x6
        # compute Kalman gain. HPH^T, project the error covariance to the measurement space.
        K = PHt @ la.inv(H @ PHt + measurement_covar)  # 卡尔曼增益   # 18x6
        ...
        q = self.nominal_state[3:7]  # 测量名义状态中的四元数

        delta = np.zeros((6, 1))  # 测量误差
    ...
    def __predict_nominal_state(self, imu_measurement: np.array):
        ...
        dt = imu_measurement[0] - self.last_predict_time  # NOTE:init timestamp
    ...
    def __predict_error_covar(self, imu_measurement: np.array):
        w_m = imu_measurement[1:4]  # 角速度
        a_m = imu_measurement[4:7]  # 加速度 ENU
