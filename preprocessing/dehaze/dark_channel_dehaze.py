...
def test_dehaze():
    test_dir = 'test_data'
    output_dir = 'dark_channel_dehazed'
    ...
    for i, path in enumerate(sorted_files, start=1):
        image = cv2.imread(path, 1)
        height = 500  # if not resize,will be very slow
        width = int(height * image.shape[0] / image.shape[1])
        image = cv2.resize(image, (height, width))
        initial, dehaze = Dehaze(image)  # 去雾

        save_path = os.path.join(output_dir, f'dehazed_{Path(path).name}')
        cv2.imwrite(save_path, dehaze)

        save_path = os.path.join(output_dir, f'combined_{Path(path).name}')
        combined = np.hstack((initial, dehaze))
        cv2.imwrite(save_path, combined)
        print(i, "dehazed:", save_path)
